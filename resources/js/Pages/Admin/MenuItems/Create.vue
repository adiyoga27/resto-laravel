...
const form = useForm({
    menu_category_id: props.categories[0]?.id ?? '',
    name: '',
    description: '',
    price: '',
    stock: 0,
...
const previewUrl = computed(() => (form.image ? URL.createObjectURL(form.image) : null));
...
<template>
    <div class="mx-auto max-w-2xl">
        <PageHeader title="Tambah Menu" subtitle="Tambahkan item menu baru" />
        <form class="card space-y-5 p-6" @submit.prevent="submit">
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                ...
                <FormGroup name="name" label="Nama Menu" :error="form.errors.name" required>
                    <input v-model="form.name" type="text" name="name" class="input" placeholder="cth: Nasi Goreng Spesial">
                </FormGroup>
            </div>
            ...
            <FormGroup name="image" label="Gambar Menu" :error="form.errors.image" hint="Maksimal 2MB">
