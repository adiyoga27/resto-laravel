...
const dateValue = String(props.cashFlow.date).slice(0, 10);

const form = useForm({
    date: dateValue,
    description: props.cashFlow.description,
    type: props.cashFlow.type,
    amount: props.cashFlow.amount,
    reference: props.cashFlow.reference ?? '',
});

const submit = () => {
    form.put(route('reports.cash-flow.update', props.cashFlow.id));
};
...
<template>
    <div class="mx-auto max-w-2xl">
        <PageHeader title="Edit Entri Arus Kas" :subtitle="`Memperbarui entri #${cashFlow.id}`" />
        <form class="card space-y-5 p-6" @submit.prevent="submit">
            ...
            <FormGroup name="description" label="Keterangan" :error="form.errors.description" required>
                <input v-model="form.description" type="text" name="description" class="input">
            </FormGroup>
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
